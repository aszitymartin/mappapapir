...
setWizardHome();
function openHeaderProfileAction (device_type) { console.log('main'); document.getElementsByTagName('html')[0].classList.add('stop-scroll', 'stop-scroll-mobile');
...
function tryLogin () { document.getElementById("wizard_next_btn").textContent = "Ellenőrzés..."; let apiKey = '739837e232548988c86b954108794b57bd3e1dbcd6eb550bfa53e544';
    $.getJSON('https://api.ipdata.co?api-key=' + apiKey, function(data) { var log__data = new FormData();
        ...
        $.ajax({ enctype: "multipart/form-data", type: "POST", url: "/actions/auth.php", data: log__data, dataType: 'json', contentType: false, processData: false,
            success: function(res) { var logerr = document.createElement('span'); logerr.classList = "small-med text-danger"; logerr.textContent = "Hibás adatokat adott meg!";
                ...
                if (res === 410) { document.getElementById("wizard_next_btn").textContent = "Belépés"; document.getElementById("login__email").value = ""; document.getElementById("login__password").value = ""; document.getElementById('logerr__con').append(logerr); }
            }, error: function (res) { var logerr = document.createElement('span'); logerr.classList = "small-med text-danger"; logerr.textContent = "Hibás adatokat adott meg!"; document.getElementById("wizard_next_btn").textContent = "Belépés"; document.getElementById("login__email").value = ""; document.getElementById("login__password").value = ""; document.getElementById('logerr__con').append(logerr); }
        });
    }); 
}
function wizardHome () { setWizardHome(); wizard_form_script.remove();}
function closeHeaderProfileAction() { document.getElementById('wizard_con').classList.remove('panelSlideUp'); document.getElementById('wizard_con').classList.add('panelSlideDown');setTimeout(function () { wrapper.remove(); profileHeaderContainer.remove(); document.body.style.overflowX = 'hidden'; document.body.style.width = '100%'; wizard_form_script.remove(); }, 340); enableSwipers();}
