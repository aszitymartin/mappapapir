    ...
    <div class="flex flex-col w-fa gap-1 border-soft item-bg box-shadow padding-1">
        <div class="flex flex-col gap-025">
            <div class="flex flex-row flex-align-c flex-justify-con-sb gap-05">
                <span class="text-primary bold">Beállítások</span>
                <div class="flex flex-row flex-align-c gap-1">
                    <div error-data="review"></div>
                    <div class="flex flex-row flex-justify-con-fe w-fa">
                        <div onclick="__saveitem(urlpid, 'review')" class="flex flex-row flex-align-c flex-justify-con-sb gap-1 primary-bg primary-bg-hover pointer user-select-none padding-05 border-soft-light">
        ...
        <div class="flex flex-col gap-2">
            <div class="flex flex-col gap-2">
                <div class="flex flex-col">
                    <div class="flex flex-row flex-align-c gap-1">
                        <label class="cst-chb-lbl">
                            <input type="checkbox" class="absolute" name="product-review-stat" id="product-review-stat">
                ...
                <div class="flex flex-col">
                    <div class="flex flex-row flex-align-c gap-1">
                        <label class="cst-chb-lbl">
                            <input type="checkbox" class="absolute" name="product-review-privacy" id="product-review-privacy">
                            <span class="cst-checkbox"></span>
                        </label>
...
<div class="flex flex-row-d-col-m gap-1">
    <div class="flex flex-col w-fa gap-1 border-soft item-bg box-shadow padding-1">
        ...
        <div class="flex flex-col gap-2">
            <table class="text-muted small-med text-align-l w-fa" id="review__table">
                <tr class="uppercase">
                    <th>Értékelés</th><th>Vásárló</th><th>Hozzászólás</th><th>Dátum</th><th>Tevékenységek</th>
                </tr>
            </table>
        </div>
    </div>
...
$.ajax({ enctype: "multipart/form-data", type: "POST", url: "/admin/products/reviews.php", data: urldata, dataType: 'json', contentType: false, processData: false,
    success: function(data) {
        for (let i = 0; i < data.length; i++) {
            var piData = new FormData(); piData.append("pid", data[i].pid);
            $.ajax({ enctype: "multipart/form-data", type: "POST", url: "/admin/products/info.php", data: piData, dataType: 'json', contentType: false, processData: false,
                success: function(pd) {
                    document.getElementById('review__table').innerHTML += `
                        <tr>
                            <td class="padding-tb-1 text-secondary bold" id="revstar__con${data[i].uid}"></td>
                            <td class="padding-tb-1 text-secondary bold">
                                <div class="flex flex-row flex-align-c gap-05">
                                    <span class="flex flex-row flex-align-c flex-justify-con-c bold text-white box-shadow curron__head circle padding-025 smaller" title="${data[i].fullname}" style="background-color: #${data[i].color}">${data[i].initials}</span>
                                    <span>${data[i].fullname}</span>
                                </div>
                            </td>
                            <td class="padding-tb-1 text-muted">${data[i].description.slice(0, 32)}...</td>
                            <td class="padding-tb-1 text-muted">${timeAgo(data[i].date)}</td>
                            <td class="padding-tb-1 relative">
                                <div class="flex flex-row flex-align-c primary-bg border-soft padding-025 stockd__trigger pointer user-select-none w-fc" id="evt__trigg${data[i].rid}" onclick="document.getElementById('evt__in${data[i].rid}').classList.toggle('stockd__show');">
                                    <span class="small-med bold">Válasszon</span>
...
<script src="/assets/script/timeago.js"></script>