<?php session_start(); require_once($_SERVER['DOCUMENT_ROOT'].'/assets/alph.php');
    ...
    $stmt = $con->prepare('SELECT customers.id, customers.fullname, customers.email, customers.phone, customers.theme, customers__money.money, customers__ship.postal, customers__ship.settlement, customers__ship.address, customers__inv.company
    ...
    $stmt->bind_result($id, $fullname, $email, $phone, $theme, $money, $postal, $settlement, $address, $company); $stmt->fetch();$stmt->close();
?>