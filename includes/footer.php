        ...
        <script content-type="application/javascript">
            ...
            function profileActionPanel (panel) {
                if (panel === 'staff') {const isStaffLock = localStorage.getItem('staffLock');const getlockParams = JSON.parse(isStaffLock);
                    if (!isStaffLock) {
                            ...
                            <div class='profileActionCon flex flex-col absolute'>
                                ...
                                <div class="flex flex-col padding-1">
                                    ...
                                    <div class="theme-button-con flex flex-col" style="background-color: transparent;">
                                        <div class="theme-item flex flex-row pointer" style="padding: .5rem 1rem;" id="staff-auth-button-desktop">
                        ...
                        `; document.getElementById('staff-auth').focus();
                    } else {
                            var c = new Date(getlockParams.lockExpiry).getTime();var t = setInterval(function() {
                                var n = new Date().getTime();var d = c - n;var da = Math.floor(d / (1000 * 60 * 60 * 24));
                                var h = Math.floor((d % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));var m = Math.floor((d % (1000 * 60 * 60)) / (1000 * 60));
                                var s = Math.floor((d % (1000 * 60)) / 1000);document.getElementById("counterLocked").innerHTML = da + ":" + h + ":"+ m + ":" + s;
        ...
        <script content-type="application/javascript"> // Fejlecben levo keresomezo beallitasa
            function mainSearch () {
                ...
                con = document.getElementById("main");item = con.getElementsByTagName("label");
            ...
            function rememberMe () { include($_SERVER['DOCUMENT_ROOT'].'/includes/inc.connect.php');
                ...
                if ($cookie) { $token = $_COOKIE['__au__login'];
                    ...
                    if ($result = mysqli_query($con, $sql)) { $num = mysqli_num_rows($result);
                        if ($num > 0) {
                            $res = mysqli_fetch_assoc($result); session_regenerate_id(); $_SESSION['loggedin'] = TRUE; $_SESSION['id'] = $res['uid']; $uid = $_SESSION['id'];
                            $pr__sql = "SELECT id, fullname, theme FROM customers WHERE id = $uid";
                            if ($pr__result = $con->query($pr__sql)) { $ulg = $pr__result->fetch_array(); $uname = $ulg['fullname']; $getTheme = $ulg['theme']; }
                            echo '<script>
                                document.getElementById("hd-rb-con").innerHTML = `
                                    ...
                                    <div class="padding-05 pointer flex flex-align-c flex-justify-con-c has-tooltip relative" id="deskBask" aria-describedby="tooltipBasket" onclick="sh__basket()">
                                        ...
                                        <span class="tooltip absolute" id="tooltipBasket"><span key="tooltipBasket" key="tooltipBasket">Kosár megtekintése</span></span>
                                    </div>
                                `;
                            ...
                            <script>
                            document.getElementById("hd-rb-con").innerHTML = `
                                <div id="header_desktop_button" class="header_heading_button user_action_button pointer flex flex-align-c flex-justify-con-c has-tooltip relative gap-05 user-select-none"  aria-describedby="tooltipProfile" onclick="openHeaderProfileAction();">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1"><g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><polygon points="0 0 24 0 24 24 0 24"/><path d="M12,11 C9.790861,11 8,9.209139 8,7 C8,4.790861 9.790861,3 12,3 C14.209139,3 16,4.790861 16,7 C16,9.209139 14.209139,11 12,11 Z" class="svg" fill-rule="nonzero" opacity="0.3"/><path d="M3.00065168,20.1992055 C3.38825852,15.4265159 7.26191235,13 11.9833413,13 C16.7712164,13 20.7048837,15.2931929 20.9979143,20.2 C21.0095879,20.3954741 20.9979143,21 20.2466999,21 C16.541124,21 11.0347247,21 3.72750223,21 C3.47671215,21 2.97953825,20.45918 3.00065168,20.1992055 Z" class="svg" fill-rule="nonzero"/></g></svg>
                                    <span class="text-primary bold small-med">Belépés</span>
                                </div>
                            `;
                            </script>
                            ';
                        }
                    } else { echo '<script>console.log("MYSQL ERROR");</script>'; }
                } else { 
                    echo '
                    <script>
                    document.getElementById("hd-rb-con").innerHTML = `
        ...
        <script src="/assets/script/classes/class.panel.js"></script>
        ...
        <noscript>
            <div class="noscript flex flex-col flex-align-c flex-justify-con-c fixed w-fa larger">
                ...
                <span style="color: #004E9C;"><strong>Figyelem!</strong> Ahhoz, hogy oldalunk megfelelően működjön, engedélyeznie kell a JavaScript működését.</span>
                <span style="color: #004E9C;">A JavaScriptet engedélyezheti a <strong>böngészője beállításaiban</strong></span>
            </div>
        </noscript>
    </body>
</html>