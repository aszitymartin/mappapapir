...
(9, 'Pesti Bela', 'kovacsjani@email.com', '$2y$10$PYIkXT6ACwm/3PgLYXMVuu1DZ6zWoPW9ygNii3yQ1FYpawwCWh78i', 301234567, 123456789, 'auto', '2022-07-30 08:31:49');
...
INSERT INTO `products__inventory` (`id`, `pid`, `code`, `quantity`, `q__warhause`, `unit`, `backorder`) VALUES
...
INSERT INTO `products__shipping` (`id`, `pid`, `physical`, `weight`, `w__unit`, `width`, `height`, `length`, `d__unit`) VALUES
(1, 1, 1, 136, 'Gramm', 8, 2, 16, 'Centiméter');
...
CREATE TABLE `products__variations` (
  ...
  `meterial` varchar(255) COLLATE utf8mb4_hungarian_ci NOT NULL,
...
INSERT INTO `products__variations` (`id`, `pid`, `color`, `meterial`, `style`, `brand`, `model`) VALUES
(1, 1, 'Fekete', 'Műanyag', 'Napelem és elem', 'Texas Instruments', 'TI-30XIIS');
...
(1, 1, 1, 'Kiváló számológép, egyetemi szintű algebrához, trighez és kémiához használom, és eddig kibírta az állandó edzést és azt, hogy a könyves táskámban hordtam, és időnként leesett. A számológépnek ez az a verziója, amellyel az egyetemi matematika tanszék lehetővé teszi a tesztek letételét. A grafikont ábrázoló \"magasabb\" szintű számológépek nem engedélyezettek.', 5, '2022-08-03 09:29:12');
...
ALTER TABLE `cart`
  ...
  ADD CONSTRAINT `cart_ibfk_2` FOREIGN KEY (`pid`) REFERENCES `old__products` (`product_id`) ON DELETE CASCADE;
...
--
ALTER TABLE `rv__r`
  ADD CONSTRAINT `rv__r_ibfk_1` FOREIGN KEY (`rid`) REFERENCES `old__reviews` (`id`) ON DELETE CASCADE,
...
--
ALTER TABLE `rv__u`
  ADD CONSTRAINT `rv__u_ibfk_1` FOREIGN KEY (`rid`) REFERENCES `old__reviews` (`id`) ON DELETE CASCADE,
